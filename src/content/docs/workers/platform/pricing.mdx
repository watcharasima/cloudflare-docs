...
If you are a Workers Enterprise customer, you may have opted to remain on the legacy Bundled or Unbound usage models. Bundled and Unbound are billed on the following metrics, rates vary based on your contract: 
| Usage Model | Requests  | Duration  | CPU time  |       
|---- | ----- | ------ | ------- |
| **Bundled** | $ / MM requests | No charge for duration | 50 milliseconds CPU time per invocation  | 
| **Unbound** | $ / MM requests | $ / MM GB-s | 30 seconds of CPU time per invocation <br /> 15 minutes of CPU time per [Cron Trigger](/workers/configuration/cron-triggers/) or [Queue Consumer](/queues/configuration/javascript-apis/#consumer) invocation |
...
3. In **Overview**, select your Worker > **Settings** > **General** > **Pricing**.
