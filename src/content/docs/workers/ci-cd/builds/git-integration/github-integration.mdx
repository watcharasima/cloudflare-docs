...
Support for building and deploying from multiple branches will be added soon, along with the ability to generate Preview URLs for pull requests (PRs).
:::

### Check run

If you have multiple Workers connected to a repository (i.e. a [monorepo](/workers/ci-cd/builds/advanced-setups/#monorepos)), you can check on the status of each build within GitHub via [GitHub check runs](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/collaborating-on-repositories-with-code-quality-features/about-status-checks#checks).

You can see the checks by selecting on the status icon next to a commit within your GitHub repository. In the example below, you can select on the green check mark to see the results of the check run.
...
You can deploy projects to Cloudflare Workers from your open-source team, company, or side project on GitHub.
...
You can remove Cloudflare Workers' access to your GitHub repository or account by viewing the [Applications page](https://github.com/settings/installations) on GitHub or going to [manage Git installation](/workers/ci-cd/builds/git-integration/#managing-a-git-installation) on the dashboard. The GitHub App is named Cloudflare Workers and Pages, and it is shared between Workers and Pages projects.
