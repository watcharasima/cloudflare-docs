...
- Check against Cloudflare Workers KV to know what events are new.
...
In this example, we will also need to use Cloudflare Workers KV to save the latest checked event IDs which allows us to know what events are new. Once you have created a Workers KV, you can head back to the `wranglers.toml` file and add the following sections:
...
In our example, we use the `env` variables to get the runtime variables like the TOKEN and ASN of interest, and Cloudflare Workers
...
We use the Cloudflare Workers KV bucket previously created and defined (`HIJACKS_KV`) to save and retrieve the most recent
processed event ID.
...
while (true) {
	...
	// first batch, save key-value value only
	if (first_batch) {
...
// update latest_id key-value value
