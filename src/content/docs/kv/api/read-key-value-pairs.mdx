...
The following methods are provided to read from Workers KV:
...
To get the value for a given key, call the `get()` method on any Workers KV namespace you have bound to your Worker code:
...
- `options`: `{
  ...
  - Optional. Object containing the optional `cacheTtl` and `type` properties. The `cacheTtl` property defines the length of time in seconds that a Workers KV result is cached in the global network location it is accessed from (minimum: 60). The `type` property defines the type of the value to be returned.
