...
For this example to work, you need to provision a [Workers KV](/kv/) namespace first. To do so, follow the [Workers KV - Get started ](/kv/get-started/) guide.

Importantly, your `wrangler.toml` file must be updated to include the `KV` binding definition to your respective namespace.

## Worker code

```ts title="Embedded function calling example with Workers KV API"
